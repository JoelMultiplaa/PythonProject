...
if not st.session_state.setup_complete:
    ...
    st.session_state["experience"] = st.text_area(
        label="Erfaring", height=None, max_chars=200, value=st.session_state["experience"],
        placeholder="Beskriv din erfaring"
    )

    st.session_state["skills"] = st.text_area(
        label="Færdigheder", height=None, max_chars=200, value=st.session_state["skills"],
    ...
    with col2:
        st.session_state["position"] = st.selectbox(
            "Vælg stilling",
            ("Frontend Udvikler", "Backend Udvikler", "Fullstack Udvikler", "Dataanalytiker", "Dataingeniør",
             "DevOps Ingeniør", "UX/UI Designer", "Projektleder", "Cloud Ingeniør", "AI-udvikler", "Mobiludvikler",
             "Softwareudvikler", "Webudvikler", "IT-konsulent")
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    st.info(
        """
        Start med at præsentere dig selv:
        """,
        icon="👋"
    )
    ...
    if not st.session_state.message:
        st.session_state.messages = [{
            "role": "system",
            "content": (f"Du er en HR-medarbejder, som interviewer en ansøger ved navn {st.session_state['name']}. "
...
if st.session_state.feedback_shown:
    ...
    feedback_completion = feedback_client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"This is the interview you need to evaluate. Keep in mind that you are only a tool. And you should"
            }
        ]
    )
